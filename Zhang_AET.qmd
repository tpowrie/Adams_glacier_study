...
for (year in names(sorted_ppt_list)) {
  for (month in names(sorted_ppt_list[[year]])) {
    ...
    sum_ppt <- sum(values(rast_ppt), na.rm = TRUE)  
    monthly_ppt <- rbind(monthly_ppt, data.frame(month, precip = sum_ppt))
  }
}
...
adams_discharge$`Discharge (mm/day)` <- ((adams_discharge$`Discharge (m3/s)` / 3137132572.000) * 86400 * 1000) * 7904.1
...
effective_precip <- effective_precip %>% 
  mutate(ppt_effective = precip - `aet (mm)`)
...
ggplot(mDMC_hydro_yr, aes(x = cumulative_precip, y = cumulative_discharge)) +
  geom_point() +
  labs(x = "Cumulative Discharge", y = "Cumulative Preip", title = "Adams mDMC")

install.packages("changepoint")
library(changepoint)

# Assume your data frame mDMC_hydro_yr contains columns cumulative_discharge and cumulative_precip

# Perform Pettitt test for detecting changepoints
pettitt_result <- cpt.mean(mDMC_hydro_yr$cumulative_discharge, test.statistic = "Pettitt")
