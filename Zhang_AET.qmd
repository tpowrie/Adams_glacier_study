...
mDMC_hydro_yr$hydro_year <- as.Date(mDMC_hydro_yr$hydro_year,)

ts_mDMC <- zoo(mDMC_hydro_yr$cumulative_discharge / mDMC_hydro_yr$cumulative_precip, mDMC_hydro_yr$hydro_year)

View(ts_mDMC)

break.point <- cpt.meanvar(ts_mDMC, method = "PELT")

BP <- attr(break.point, "cpt.meanvar")$indexes

ggplot(mDMC_hydro_yr, aes(x = cumulative_precip, y = cumulative_discharge, color = hydro_year)) +
  geom_point(size = 1) +
  scale_color_gradient(low = "blue", high = "red") +
  geom_vline(xintercept = mDMC_hydro_yr$cumulative_precip[BP], linetype = "dashed") +
  labs(x = "Cumulative Effective Precipitation (mm)",
...
seg.model <- lm(cumulative_discharge ~ cumulative_precip, data = mDMC_hydro_yr)
seg.fit <- segmented(seg.model, seg.Z = ~ cumulative_precip)

# plot the segmented regression line with data
plot(cumulative_discharge ~ cumulative_precip, data = mDMC_hydro_yr, pch = 16, cex = 0.5, col = "gray")
lines(seg.fit, col = "red", lwd = 2)

# add vertical line for the break point
abline(v = seg.fit$psi, lty = "dashed")

# show the segmented regression summary
summary(seg.fit)
...
diffs <- mDMC_hydro_yr$cumulative_discharge - mDMC_hydro_yr$cumulative_precip

# Compute the Pettitt test statistic and p-value
pettitt_result <- pettitt.test(diffs)
test_statistic <- pettitt_result$statistic
p_value <- pettitt_result$p.value

# Plot the data and the breaking point
plot(mDMC_hydro_yr$cumulative_discharge ~ mDMC_hydro_yr$cumulative_precip)
abline(v = which.max(abs(diff(diffs))) + 1, col = "red")
```
